...
class MinimumDataSizeExperiment:
    ...
    def find_minimum_data_size(
        ...
        while current_size <= max_size:
            print(
                f"Testing {model_type} on {distribution_type} distribution in {dimension}D with {current_size} samples"
            )
            ...
            if model_type == "maf":
                ...
                target = experiment.create_target_distribution(
                    dimension, distribution_type
                )
                data_splits = experiment.generate_data_splits(
                    dimension, current_size, target, distribution_type
                )

                # Create and train model
                model = experiment.create_flow_model(
                    dimension, params, distribution_type
                )
                experiment.train_model(
                    dimension, model, target, data_splits, params, distribution_type
                )

            else:  # KDE model
                experiment = KDEExperiment(
                    output_file="temp_results.xlsx", seed=self.seed
                )
                ...
                experiment.train_and_select_bandwidth(
                    dimension, model, data_splits, distribution_type
                )

            # Evaluate model to get KL divergence
            evaluation_results = experiment.evaluate_model(
                dimension, model, target, data_splits, distribution_type
            )
    ...
    def _save_result(self, result):
        ...
        if os.path.exists(self.output_file):
            ...
            mask = (
                (existing_df["model_type"] != result["model_type"])
        ...
        print(
            f"Result saved for {result['model_type']}, {result['distribution']}, dimension {result['dimension']}"
        )
            ...
            if model_type == "kde":
                for distribution_type in ["gaussian", "mixture", "skewnormal"]:
                    for dimension in range(1, 8):
                        print(f"\n{'='*60}")
                        print(
                            f"Finding minimum data size for {model_type}, {distribution_type}, dimension {dimension}"
                        )
                        print(f"{'='*60}")
    ...
    def visualize_results(self):
        ...
        for dist in ["gaussian", "mixture", "skewnormal"]:
            summary_table[(dist, "ratio")] = (
                summary_table[(dist, "kde")] / summary_table[(dist, "maf")]
            )
