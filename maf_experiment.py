...
class Mixture_of_Gaussians:
    ...
    def log_prob(self, z):
        ...
        log_probs = log_probs - 0.5 * torch.sum(
            ((z_expanded - means_expanded) / stds_expanded) ** 2, dim=-1
        )
...
class Skew_Normal:
    ...
    def log_prob(self, z):
        ...
        cdf_term = torch.where(
            ...
            0.5
            + 0.5
            * torch.sign(cdf_input)
            * (1 - torch.exp(-0.7 * torch.abs(cdf_input))),
        )
    ...
    def sample(self, num_samples):
        ...
        z = (
            delta.unsqueeze(0) * torch.abs(u0)
            + torch.sqrt(1 - delta**2).unsqueeze(0) * u1
        )
...
class MAF_Multidimensional_Comparison_Experiment:
    ...
    def create_target_distribution(self, dim: int, distribution_type="gaussian"):
        ...
        if distribution_type == "gaussian":
            ...
            target = Gaussian_Distribution(dim=dim, mean=mean, std=std)
    ...
    def generate_data_splits(
        self, dim: int, total_size: int, target, distribution_type="gaussian"
    ):
        """
        ...
        for i in range(K):
            flows += [
                nf.flows.MaskedAffineAutoregressive(
                    dim, hidden_units, num_blocks=num_blocks
                )
            ]
    ...
    def train_model(
        ...
        for it in tqdm(
            range(max_iter),
            desc=f"Training... {dim}D model ({distribution_type}) distribution",
        ):
            model.train()
            optimizer.zero_grad()
            ...
            if it % 10 == 0:
                ...
                with torch.no_grad():
                    ...
                    if no_improvement >= patience:
                        print(
                            f"Early stopping at iteration {it}. Best iteration: {best_iteration}"
                        )
                        break
    ...
    def evaluate_model(
        self, dim: int, model, target, data_splits: dict, distribution_type="gaussian"
    ):
        """
        ...
        with torch.no_grad():
            ...
            sample_quality = self.evaluate_sample_quality(
                model, target, large_test_data.shape[0]
            )
        ...
        with torch.no_grad():
            ...
            if model_samples.shape[1] <= 4:  # Only compute for dim <= 4
                ...
                if model_samples.shape[1] == 1:
                    ...
                    wasserstein_approx = torch.mean(
                        torch.abs(sorted_model - sorted_target)
                    ).item()
                else:
                    # For higher dimensions, use a simple distance-based approximation
                    wasserstein_approx = torch.norm(
                        model_mean - target_mean
                    ) + torch.norm(model_std - target_std)
    ...
    def plot_training_loss(
        ...
        plt.plot(
            np.arange(0, len(train_loss_hist), 10)[: len(val_loss_hist)],
            val_loss_hist,
        ...
        plt.ylabel("Loss")
        plt.title(
            f"Training and Validation Loss for {dim}D Model ({distribution_type}) dsitributiob"
        )
    ...
    def save_results_to_excel(self, results_dict):
        ...
        try:
            ...
            if os.path.exists(self.output_file):
                ...
                if (
                    "dimension" in existing_df.columns
                    and "distribution_type" in existing_df.columns
                ):
                    mask = (existing_df["dimension"] != results_dict["dimension"]) | (
                        existing_df["distribution_type"]
                        != results_dict["distribution_type"]
                    )
            ...
            print(
                f"Results for {results_dict['dimension']}D model ({results_dict['distribution_type']}) saved to {self.output_file}"
            )
    ...
    def run_single_experiment(self, dim: int, distribution_type="gaussian"):
        ...
        print(
            f"Running experiment for dimension {dim} with {distribution_type} distribution (seed: {experiment_seed})"
        )
        ...
        data_splits = self.generate_data_splits(
            dim, params["total_data_size"], target, distribution_type
        )
        ...
        training_results = self.train_model(
            dim, model, target, data_splits, params, distribution_type
        )

        # Evaluate model on test set
        evaluation_results = self.evaluate_model(
            dim, model, target, data_splits, distribution_type
        )
    ...
    def run_experiments(self, dimensions=None, distribution_types=None):
        ...
        if dimensions is None:
            ...
            raise ValueError(
                "Invalid dimensions parameter. Must be None, int, tuple(start,end), or list."
            )
        ...
        if distribution_types is None:
            ...
            if distribution_types not in all_distributions:
                raise ValueError(
                    f"Invalid distribution type. Must be one of {all_distributions}"
                )
            dists_to_run = [distribution_types]
        elif isinstance(distribution_types, list):
            invalid_dists = [
                d for d in distribution_types if d not in all_distributions
            ]
            if invalid_dists:
                raise ValueError(
                    f"Invalid distribution types: {invalid_dists}. Must be among {all_distributions}"
                )
            dists_to_run = distribution_types
        else:
            raise ValueError(
                "Invalid distribution_types parameter. Must be None, str, or list of strings."
            )
...
if __name__ == "__main__":
    ...
    experiment = ImprovedMultidimensionalComparisonExperiment(
        output_file="normalizing_flow_results_nonconditional.xlsx", seed=42
    )
