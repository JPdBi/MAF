...
class KDEMixtureOfGaussians:
    ...
    def __init__(self, dim, num_components=3, means=None, stds=None, weights=None):
        ...
        if stds is None:
            self.stds = [
                torch.ones(dim, device=self.device) * 0.5 for _ in range(num_components)
            ]
        else:
        ...
        if weights is None:
            self.weights = (
                torch.ones(num_components, device=self.device) / num_components
            )
        else:
    ...
    def log_prob(self, x):
        ...
        for i in range(self.num_components):
            log_probs[:, i] = self.components[i].log_prob(x) + torch.log(
                self.weights[i]
            )
...
class KDE:
    ...
    def select_bandwidth(self):
        ...
        print(
            f"Initial bandwidth estimates: Silverman={h_silverman:.5f}, Scott={h_scott:.5f}, IQR-based={h_iqr:.5f}"
        )
        ...
        n_grid = 20
        grid_log_h = torch.linspace(
            torch.log(torch.tensor(h_min)), torch.log(torch.tensor(h_max)), n_grid
        )
        ...
        print(
            f"Coarse grid search: best log(h)={best_log_h_coarse:.5f}, score={-best_score:.5f}"
        )
        ...
        fine_grid_log_h = torch.linspace(
            torch.tensor(fine_log_h_min), torch.tensor(fine_log_h_max), n_fine_grid
        )
        ...
        print(
            f"Fine grid search: best log(h)={best_log_h_fine:.5f}, score={-best_score:.5f}"
        )
...
class KDEExperiment:
    ...
    def generate_data_splits(
        self, dim, total_size, target, distribution_type="gaussian"
    ):
        """
    ...
    def train_and_select_bandwidth(
        self, dim, model, data_splits, distribution_type="gaussian"
    ):
        """
        ...
        training_results = {
            "best_bandwidth": (
                best_bandwidth.item()
                if hasattr(best_bandwidth, "item")
                else float(best_bandwidth)
            ),
    ...
    def evaluate_model(
        self, dim, model, target, data_splits, distribution_type="gaussian"
    ):
        """
    ...
    def run_single_experiment(self, dim, distribution_type="gaussian"):
        ...
        print(
            f"Running KDE experiment for dimension {dim} with {distribution_type} distribution"
        )
        ...
        total_data_size = base_data_size + (
            2 * base_data_size * (dim - 1)
        )  # Linear scaling with dimension

        # Generate data splits and train
        data_splits = self.generate_data_splits(
            dim, total_data_size, target, distribution_type
        )
        model = KDE(dim=dim)

        # Train and select bandwidth
        training_results = self.train_and_select_bandwidth(
            dim, model, data_splits, distribution_type
        )
